...
{{"score": <number 0-10>, "issue": "<brief explanation if score < 8, empty string otherwise>"}}"""
            ...
            response = await self.client.completions.create(  # type: ignore
                model="gpt-3.5-turbo-instruct",  # Switched to high-throughput instruct model
                prompt=prompt,
                temperature=0,
                max_tokens=100,  # A bit more room for the completion model
            )
        ...
        except (httpx.TimeoutException, asyncio.TimeoutError) as e:  # Catch specific timeout errors
            print(f"  ⚠️ LLM grammar check timed out: {e}. Returning neutral score (5.0).")
            return 5.0, [f"LLM grammar check timed out: {e}"]
        except Exception as e:  # Catch other unexpected errors
            print(
                f"  ⚠️ LLM grammar check failed with unexpected error: {e}. Returning neutral score (5.0)."
            )
